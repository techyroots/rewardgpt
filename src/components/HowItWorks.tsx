import { cashbackCents, formatUsd, getServiceDefinition } from "@/lib/services";
import { ChatGptMark } from "./logos";

const STEPS = [
  {
    title: "Connect Wallet",
    body: "Link your wallet, or have one created for you, to get started.",
  },
  {
  ...
  },
  {
    ...
    body: "Receive your rewards directly to your wallet. No gas, no signature.",
  },
];

export function HowItWorks() {
  const example = getServiceDefinition("chatgpt");
  const reward = cashbackCents(example.priceUsdCents, example.cashbackBps);
  ...
  return (
    <section id="how-it-works" className="mx-auto max-w-6xl px-4 py-12 sm:px-6">
      ...
      <div className="mt-6 grid gap-8 lg:grid-cols-[1.35fr_1fr] lg:items-start">
        ...
        <div id="rewards" className="rounded-2xl border border-line bg-surface p-5 card-float">
          ...
          <div className="mt-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium">{example.planLabel}</p>
          ...
          <div className="mt-5 flex items-end justify-between border-t border-line pt-4">
            <div>
              ...
              <p className="mt-0.5 text-xl font-semibold tracking-tight">
                {example.cashbackBps / 100}%
              </p>
            </div>
            <div className="text-right">
              <p className="text-[12.5px] text-muted">Eligible Reward</p>
              <p className="mt-0.5 text-xl font-semibold tracking-tight">
                {formatUsd(reward)} <span className="text-[12px] font-normal text-muted">USDC</span>
              </p>
